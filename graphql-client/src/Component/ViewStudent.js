...
import { GetAllStudent } from '../Action/SchoolAction'

export default function UserListing() {

    const dispatch = useDispatch();

    const {sch }= useSelector((state) => state.AllSchools);
    console.log("Hi", sch);
    const { data, loading } = useQuery(FETCH_USER);
    console.log("welcoome", data);
    const [school, setSchool] = useState(data);

    useEffect(() => {
        // if (sch.school) {
        //         setSchool(sch.school);
        //  dispatch(getAllStudents(sch.school));
        // console.log("ki",school);
                        ...
                        <div className="conatiner">
                            <div className="row">
                                <table className="table table-light table-stripped table-hover m-auto">
                                    ...
                                    <tbody>
                                        {
                                            school && sch.school.school.map((list, key) => {
                                                return (
                                                    <tr key={key}>
                                                        <td>{list.id}</td>
